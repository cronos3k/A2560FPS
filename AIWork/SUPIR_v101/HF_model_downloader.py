...
# Configuration
TARGET_DIR = "SUPIR/models"

DOWNLOAD_CONFIG = {
    "repos": [
        {
            "repo_id": "MonsterMMORPG/SECourses_SUPIR",
            "local_dir": "SUPIR/models",
            "file_filters": []
        }
    ],
...
def download_models(download_dir: Optional[str] = None, minimal: bool = False):
    """Main download function"""
    target_dir = download_dir if download_dir else TARGET_DIR

    if minimal:
        print(f"\nDownloading SUPIR models (MINIMAL MODE)...")
        print(f"Will download: RealVisXL_V5.0_fp16.safetensors checkpoint only")
        print(f"Will skip: other checkpoints (Juggernaut, WildCard), LLaVA models")
    else:
        print(f"\nDownloading SUPIR models (FULL MODE)...")

    downloader = RobustDownloader(DOWNLOAD_CONFIG)

    os.makedirs(target_dir, exist_ok=True)

    for repo_config in DOWNLOAD_CONFIG["repos"]:
        repo_id = repo_config["repo_id"]

        print(f"\nRepository: {repo_id}")
        print(f"Target: {os.path.abspath(target_dir)}")

        files = scan_repo_files(repo_id)

        if not files:
            print("No files found")
            continue

        # Apply minimal mode filtering
        if minimal:
            filtered_files = []
            for filename in files:
                # Skip LLaVA models
                if filename.startswith('llava-v1.5-7b/'):
                    continue
                # Skip other checkpoint files, only keep RealVisXL_V5.0_fp16.safetensors
                if filename.startswith('checkpoints/'):
                    if filename == 'checkpoints/RealVisXL_V5.0_fp16.safetensors':
                        filtered_files.append(filename)
                    # Skip other checkpoint files
                    continue
                # Include all other files
                filtered_files.append(filename)
            files = filtered_files
            print(f"Minimal mode: filtered to {len(files)} files (skipped LLaVA and other checkpoints)")
            ...
            if os.path.exists(filepath) and os.path.getsize(filepath) > 1024:
                ...
                if expected_sha:
                    ...
                    if downloader.is_file_verified(repo_id, filename, filepath, expected_sha):
                        print(f"[SKIP] {filename} already verified (cached)")
                        skipped.append(filename)
                    elif downloader.verify_file_sha256(filepath, expected_sha, filename):
                        # Mark as verified after successful verification
                        downloader.mark_file_verified(repo_id, filename, filepath, expected_sha)
                        skipped.append(filename)
                    else:
                        print(f"[RE-DOWNLOAD] {filename} failed verification")
        ...
        for i, filename in enumerate(to_download, 1):
            print(f"\n[{i}/{len(to_download)}]")
            if downloader.download_file(repo_id, filename, target_dir):
                successful += 1
            else:
                failed += 1
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Download SUPIR models')
    parser.add_argument('--dir', type=str,
                       help='Download directory')
    parser.add_argument('--minimal', action='store_true',
                       help='Download minimal set: only RealVisXL_V5.0_fp16.safetensors checkpoint and skip LLaVA models')

    args = parser.parse_args()
    download_models(args.dir, minimal=args.minimal)